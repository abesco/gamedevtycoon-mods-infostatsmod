    ...
    this.getRatingStarsAsDivElement = function(score, showText, scaleTo) {
        ...
        for(var i = 1; i < 11; i++)
        {
            ...
            var dirPath = path.join(
                ...
                './mods/InfoStatsMod/img/'
            );

            if ( i <= ful ) {
                $img.attr('src', path.join(dirPath, './rating_4.png'));
            }
            else if ( i == ful + 1 && dec > 0){
                if (dec >= 75){
                    $img.attr('src', path.join(dirPath, './rating_3.png'));
                }
                else if(dec >= 50 || dec == 5){
                    $img.attr('src', path.join(dirPath, './rating_2.png'));
                }
                else if(dec >= 25){
                    $img.attr('src', path.join(dirPath, './rating_1.png'));
                }
            }
            else {
                $img.attr('src', path.join(dirPath, './rating_0.png'));
            }
